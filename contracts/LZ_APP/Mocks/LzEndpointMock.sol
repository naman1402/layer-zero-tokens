...
contract LZEndpointMock {
    ...
    function hasStoredPayload(uint16 _srcChainId, bytes calldata _path) external view returns (bool) {
        StoredPayload storage sp = storedPayload[_srcChainId][_path];
    ...
    function forceResumeReceive(uint16 _srcChainId, bytes calldata _path) external {
        StoredPayload storage sp = storedPayload[_srcChainId][_path];
